...
import urllib.request # Added for PDB download
        ...
        with open(map_file_path, 'r') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if not parts or line.startswith("#"):
                    continue
                if len(parts) == 3: # Expecting PDB_RES_NAME PDB_RES_NUM SIM_RES_NUM
                    try:
                        pdb_res_num = int(parts[1]) # B-factor PDB residue number
                        sim_res_num = int(parts[2]) # Simulation (XVG) residue number
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot multiple XVG files, optionally overlaying B-factors from one or more PDB chains (mapped or unmapped) on a secondary Y-axis.",
    ...
    parser.add_argument('--residue_map_files', type=str, nargs='+', 
                        help="Corresponding .rmp map files for each chain in --bfac_chains. Order must match. "
                             "If provided, all B-factor chains are mapped. If omitted, B-factors are plotted with original PDB residue numbers.")
    ...
    for i, xvg_filepath_str in enumerate(args.inputs):
        ...
        if i == 0: # Capture labels from the first successfully read XVG
    ...
    if args.bfac_pdb_id and args.bfac_chains:
        ...
        if args.residue_map_files and len(args.residue_map_files) != len(args.bfac_chains):
            print("Error: Number of --residue_map_files must match number of --bfac_chains if --residue_map_files is provided.", file=sys.stderr)
            sys.exit(1)

        awk_script_full_path = Path(args.awk_script_dir) / "extract_bfactors.awk"
        
        bfactor_styles = [
            {'color': 'dimgray', 'linestyle': ':', 'marker': '.'}, # Changed marker for better visibility
            {'color': 'darkslateblue', 'linestyle': '--', 'marker': '.'},
            {'color': 'darkgreen', 'linestyle': '-.', 'marker': '.'},
            {'color': 'maroon', 'linestyle': ':', 'marker': '.'},
        ] 
        common_style_params = {'markersize': 3.5, 'alpha': 0.7, 'linewidth': 1.2} # Increased markersize
        ...
        for i, chain_id in enumerate(args.bfac_chains):
            ...
            chain_mapped_status = "orig. PDB res." # Default status

            if args.residue_map_files: # Mapping is requested for this chain
                current_map_file = args.residue_map_files[i]
                print(f"Using residue map file: {current_map_file} for chain {chain_id}")
                res_map_bfac_pdb_to_xvg = parse_residue_map_rmp(current_map_file)
                ...
                if res_map_bfac_pdb_to_xvg:
                    ...
                    for res_idx, res_num_from_bfac_pdb in enumerate(original_bfac_res):
                        if res_num_from_bfac_pdb in res_map_bfac_pdb_to_xvg:
                            mapped_res_for_xvg_axis.append(res_map_bfac_pdb_to_xvg[res_num_from_bfac_pdb])
                    ...
                    if mapped_res_for_xvg_axis:
                        ...
                        print(f"Successfully mapped {len(mapped_vals)} B-factors for chain {chain_id} using {current_map_file}.")
                        if unmapped_count > 0:
                            print(f"Note: {unmapped_count} B-factor residues from chain {chain_id} were not found in {current_map_file}.")
                    else:
                        print(f"Warning: No B-factors could be mapped for chain {chain_id} using {current_map_file}. Plotting with original residue numbers.")
                else:
                    print(f"Warning: Could not parse map file {current_map_file} for chain {chain_id}. Plotting with original residue numbers.")
            
            bfactor_legend_label = f"B-factors ({args.bfac_pdb_id} Ch.{chain_id}"
            if args.residue_map_files: # Only add mapped status if mapping was attempted
            ...
            if bfactor_x_for_plotting.size > 0:
                if not ax2: 
                    ...
                    ax2.grid(True, alpha=0.2, axis='y', linestyle=':', color='lightgrey') # Lighter grid for secondary

                current_style = next(style_cycler_bfac)
                ax2.tick_params(axis='y', labelcolor=current_style['color']) # Color ticks to match line
        ...
        if any_bfactors_plotted and bfactor_info_for_title:
            plot_title += f" (B-factors: {args.bfac_pdb_id} " + ", ".join(bfactor_info_for_title) + ")"


    ax1.set_title(plot_title, fontsize='medium')
    
    # Consolidate legends if ax2 exists
    if ax2:
        # Forcing all legend items into one box. Matplotlib might place it automatically.
        # For more control, you might need to manually specify loc and bbox_to_anchor.
        ax1.legend(lines_for_legend, labels_for_legend, loc='best', fontsize=args.legend_fontsize)
    else:
        ax1.legend(loc='best', fontsize=args.legend_fontsize)
