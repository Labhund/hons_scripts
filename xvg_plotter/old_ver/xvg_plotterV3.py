...
def parse_xvg_file(filepath):
    ...
    for line in lines:
        ...
        if line.startswith('@'): # General handler for metadata lines
            # Specific, common GROMACS headers
            if line.startswith('@    title'): # Note: GROMACS often uses 4 spaces
                ...
                # If not one of the above, try to match a legend line
                # Regex is robust to number of spaces after '@' and around 'sX'
                ...
                # else:
                    # print(f"DEBUG: Unhandled @ line: {line}") # For other @ lines like @TYPE, @view etc.
...
def plot_with_gaussian(data, y_legends, title, xlabel, ylabel, input_filename_stem):
    ...
    for y_col_idx in range(1, data.shape[1]): # y_col_idx is 1 for data[:,1], 2 for data[:,2], etc.
        ...
        legend_label = y_legends[legend_idx] # y_legends should be correctly populated now
    ...
    for y_col_idx in range(1, data.shape[1]):
        ...
        legend_label = y_legends[legend_idx] # y_legends should be correctly populated
...
def main():
    ...
    if is_ramachandran_file and data.shape[1] >= 2:
        ...
        has_meaningful_legends = any(y_legends) 
        if has_meaningful_legends or (data.shape[1] > 2 and not all(leg.startswith("Y-Col") for leg in y_legends)):
             plt.legend()
        elif data.shape[1] > 2 : 
            plt.legend()
